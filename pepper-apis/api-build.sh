...
function render_config() {
  ...
  INPUT_DIR=config DOCS_PROXIED_HOST=$DOCS_PROXIED_HOST NGINX_PROXIED_HOST=$NGINX_PROXIED_HOST OUTPUT_DIR=$BUILD_OUTDIR GCP_PROJECT=$GCP_PROJECT ENVIRONMENT=$ENVIRONMENT VERSION=$VERSION DIR=$DIR GAE=$GAE APP=$APPLICATION MANIFEST=build-manifest.rb  ruby configure.rb -y

  if [[ "$1" == "local" ]] || [[ "$1" == "gae" ]]; then
      echo "rendering runtime configs locally"
      INPUT_DIR=config NO_SYSLOG=true DOCS_PROXIED_HOST=$DOCS_PROXIED_HOST NGINX_PROXIED_HOST=$NGINX_PROXIED_HOST OUTPUT_DIR=$OUTDIR GCP_PROJECT=$GCP_PROJECT ENVIRONMENT=$ENVIRONMENT DIR=$DIR/$OUTDIR VERSION=$VERSION GAE=$GAE APP=$APPLICATION ruby configure.rb -y
