...
public class HideActivitiesEventActionTest extends TxnAwareBaseTest {
    ...
    public void test_noTargetActivityInstances() {
        TransactionWrapper.useTxn(handle -> {
            ...
            var signal = new EventSignal(testData.getUserId(), testData.getUserId(), testData.getUserGuid(),
                    testData.getUserGuid(), testData.getStudyId(), testData.getStudyGuid(), EventTriggerType.GOVERNED_USER_REGISTERED);
    ...
    @Test
    public void test_targetActivityInstancesAreHidden() {
        TransactionWrapper.useTxn(handle -> {
            ...
            var signal = new EventSignal(testData.getUserId(), testData.getUserId(), testData.getUserGuid(),
                    testData.getUserGuid(), testData.getStudyId(), testData.getStudyGuid(), EventTriggerType.GOVERNED_USER_REGISTERED);
            var action = new HideActivitiesEventAction(null, Set.of(act1.getActivityId(), act2.getActivityId()));
            action.doAction(null, handle, signal);

            var jdbiInstance = handle.attach(JdbiActivityInstance.class);
            assertTrue(jdbiInstance.getByActivityInstanceId(instance1.getId()).get().isHidden());
            assertTrue(jdbiInstance.getByActivityInstanceId(instance2.getId()).get().isHidden());
    ...
    public void test_multipleInstancesForTargetActivity() {
        TransactionWrapper.useTxn(handle -> {
            ...
            var signal = new EventSignal(testData.getUserId(), testData.getUserId(), testData.getUserGuid(),
                    testData.getUserGuid(), testData.getStudyId(), testData.getStudyGuid(), EventTriggerType.GOVERNED_USER_REGISTERED);
            var action = new HideActivitiesEventAction(null, Set.of(act1.getActivityId()));
            action.doAction(null, handle, signal);
    ...
    public void test_nonTargetedActivityInstancesNotAffected() {
        TransactionWrapper.useTxn(handle -> {
            ...
            var jdbiInstance = handle.attach(JdbiActivityInstance.class);
            assertTrue(jdbiInstance.getByActivityInstanceId(instance1.getId()).get().isHidden());
            assertFalse(jdbiInstance.getByActivityInstanceId(instance2.getId()).get().isHidden());
